...
export class RoomController {
    ...
    #quality = "hrtf";
    #disposed = false;
    constructor(context, core, destination) {
    ...
    applyPreset(preset, rampMs = 120) {
        ...
        const cutoff = Math.max(300, normalized.toneHz * (1 - normalized.highFrequencyAbsorption * 0.45));
    ...
    setReverbAmount(value, rampMs = 80) {
        this.#assertAvailable();
        this.#preset = { ...this.#preset, wet: clamp01(value) };
        if (this.#lane)
            automate(this.#lane.gain.gain, this.#targetWet(), this.#context, rampMs);
    }
    setQuality(quality, rampMs = 100) {
        this.#assertAvailable();
    ...
    importPreset(json, rampMs = 120) {
        ...
        const normalized = this.#core.normalizeRoomPreset(parsed);
    ...
    dispose() {
        ...
        if (this.#lane)
            this.#disconnectLane(this.#lane);
        for (const lane of this.#retiringLanes)
    ...
    #createImpulse(preset) {
        ...
        const earlySamples = Math.max(1, Math.floor(sampleRate * Math.min(0.08, 0.006 + preset.size * 0.07)));
    ...
    #assertAvailable() {
        if (this.#disposed)
            throw new Error("Room controller has been disposed");
    }
}
